...
def choosePivot(array, begin, end):
    arrayLen = len(array[begin:end + 1])
    samp = []
    samp.append((array[begin], begin))
    samp.append((array[(begin + end) // 2], (begin + end) // 2))
    samp.append((array[end], end))
    samp.sort()
    return samp[1][1]
